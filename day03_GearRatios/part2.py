...
class GearRatios:
    ...
    def is_gear(self, row_index, ele_index):
        ele_range_min = ele_index - 1
        ele_range_max = ele_index + 1
        row_range_min = row_index - 1
        row_range_max = row_index + 1
        ...
        for row in range(row_range_min, row_range_max + 1):
            for ele in range(ele_range_min, ele_range_max + 1):
                if self.schematic[row, ele] in "0123456789":
                    count = count + 1
        print(count)
...
def main(argv):
    ...
    gear_ratios.print()
    ans = gear_ratios.get_star_loc()
    print(ans)
    for r, c in ans:
        gear_ratios.is_gear(r, c)
